...
def validate_v11_2_release_manifest(path: Path) -> dict[str, Any]:
    try:
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: validate_frozen_release_manifest_v11_2.py <path>", file=sys.stderr)
        return 2
    result = validate_v11_2_release_manifest(Path(sys.argv[1]))
