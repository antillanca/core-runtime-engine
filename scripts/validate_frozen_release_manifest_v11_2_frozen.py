#!/usr/bin/env python3
"""Validate the frozen CORE v11.2.1 ContractProgram release surface."""
...
from scripts.validate_frozen_release_manifest_v11_2 import (  # noqa: E402
    _timezone_timestamp,
    required_v11_2_candidate_artifacts,
)

SCHEMA_VERSION = "core.frozen_release_manifest.v3"
SCHEMA_PATH = PROJECT_ROOT / "schemas" / "core" / "frozen_release_manifest.v3.json"
RELEASE_VERSION = "v11.2.1"
INVENTORY_PROFILE = "core.contract_program_release.v11_2"
...
def required_v11_2_frozen_artifacts() -> dict[str, str]:
    expected = required_v11_2_candidate_artifacts()
    expected.update(
        {
            "examples/frozen_release_manifest/accepted_v11_2_1_candidate.json": "example",
            "schemas/core/frozen_release_manifest.v3.json": "schema",
            "scripts/build_frozen_release_manifest_v11_2_frozen.py": "script",
            "scripts/validate_frozen_release_manifest_v11_2_frozen.py": "script",
            "docs/releases/v11.2.1.md": "documentation",
            "tests/test_frozen_release_manifest_v11_2_frozen.py": "test",
        }
    )
    return expected


def build_v11_2_frozen_manifest(frozen_at: str) -> dict[str, Any]:
    if not _timezone_timestamp(frozen_at):
        raise ValueError("frozen_at must include an explicit timezone")
    expected = required_v11_2_frozen_artifacts()
    artifacts = []
...
def validate_v11_2_frozen_release_manifest(path: Path) -> dict[str, Any]:
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        errors = [error("file_not_found", "Frozen v11.2 release manifest does not exist.", "path")]
        payload = {}
    except json.JSONDecodeError:
        errors = [error("invalid_json", "Frozen v11.2 release manifest is not valid JSON.", "path")]
        payload = {}
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(artifacts, list):
                ...
                if missing or unexpected:
                    errors.append(error("artifact_inventory_mismatch", "Paths must equal the frozen v11.2 inventory.", "artifacts", missing=missing, unexpected=unexpected))
    ...
    return {"schema": "core.frozen_release_manifest_validation.v3", "status": "passed" if not errors else "failed", "errors": errors, "release_version": payload.get("release_version") if isinstance(payload, dict) else None, "inventory_profile": payload.get("inventory_profile") if isinstance(payload, dict) else None, "artifact_count": payload.get("artifact_count") if isinstance(payload, dict) else None, "manifest_valid": not errors}


def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: validate_frozen_release_manifest_v11_2_frozen.py <path>", file=sys.stderr)
        return 2
    result = validate_v11_2_frozen_release_manifest(Path(sys.argv[1]))
