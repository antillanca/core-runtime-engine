...
def test_candidate_builder_is_deterministic_and_live_verification_is_explicit(tmp_path: Path) -> None:
    ...
    assert validate_v11_3_release_manifest(current, verify_live_artifacts=True)["status"] == "passed"
    assert validate_v11_3_release_manifest(ACCEPTED, verify_live_artifacts=True)["status"] == "passed"
